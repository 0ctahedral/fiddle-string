...
let get_tag_S s = match s with
  | SForall(_, _, t) -> t
;;
let get_tag_T t = match t with
  | TyBlank t -> t
  | TyCon(_, t) -> t
  | TyVar(_, t) -> t
  | TyArr(_, _, t) -> t
...
and map_tag_B (f : 'a -> 'b) b =
  ...
  | BName(x, allow_shadow, t, ax) ->
     let tag_ax = f ax in
     let tag_t = map_tag_T f t in
     BName(x, allow_shadow, tag_t, tag_ax)
  | BTuple(binds, t) ->
     let tag_tup = f t in
...
and untagE e =
  ...
  | EPrim1(op, opt_typs, e, _) ->
     EPrim1(op, map_opt untagTs opt_typs, untagE e, ())
  | EPrim2(op, opt_typs, e1, e2, _) ->
     EPrim2(op, map_opt untagTs opt_typs, untagE e1, untagE e2, ())
  | ELet(binds, body, _) ->
...
and untagB b =
  ...
  | BName(x, allow_shadow, typ, _) -> BName(x, allow_shadow, untagT typ, ())
