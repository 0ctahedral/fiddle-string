...
let make_only_namebind(name, typ, loc) =
  if name = "_"
  then raise (ParseError (Printf.sprintf "Underscore not allowed in let-rec at %s" (string_of_sourcespan loc)))
  else BName(name, typ, loc)
%}
...
namebindings :
  | namebind EQUAL expr { [($1, $3, (Parsing.rhs_start_pos 1, Parsing.rhs_end_pos 3))] }
  | namebind EQUAL expr COMMA namebindings { ($1, $3, (Parsing.rhs_start_pos 1, Parsing.rhs_end_pos 3))::$5 }
...
simple_expr :
  ...
  // Simple cases
  | const { $1 }
  | LPARENNOSPACE expr COLON typ RPAREN { EAnnot($2, $4, (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
  | LPARENSPACE expr COLON typ RPAREN { EAnnot($2, $4, (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
  | id { $1 }
...
namebind :
  | ID %prec SEMI { make_only_namebind($1, TyBlank(Parsing.symbol_start_pos (), Parsing.symbol_end_pos ()), (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
  | ID COLON typ { make_only_namebind($1, $3, (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
...
arrowtyp :
  ...
  | LPARENNOSPACE THINARROW typ RPAREN { TyArr([], $3, (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
  | LPARENSPACE THINARROW typ RPAREN { TyArr([], $3, (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
...
tupletyp :
  ...
  | LPARENNOSPACE RPAREN { TyTup([], (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
  | LPARENSPACE RPAREN { TyTup([], (Parsing.symbol_start_pos (), Parsing.symbol_end_pos ())) }
