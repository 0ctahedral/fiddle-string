...
%token <string> ID
%token <string> STRING
%token DEF ANDDEF ADD1 SUB1 LPARENSPACE LPARENNOSPACE RPAREN LBRACK RBRACK LET IN EQUAL COMMA PLUS CONCAT MINUS TIMES IF COLON ELSECOLON EOF PRINTSTACK TRUE FALSE ISSTRING PRINTF SUBSTRING LEN ISBOOL ISNUM ISTUPLE EQEQ LESSSPACE GREATER LESSEQ GREATEREQ AND OR NOT COLONEQ SEMI NIL LAMBDA BEGIN END SHADOW REC UNDERSCORE

%right SEMI
%left COLON COLONEQ
%left PLUS CONCAT MINUS TIMES GREATER LESSSPACE GREATEREQ LESSEQ EQEQ AND OR
%left LPARENNOSPACE
...
substr:
  | SUBSTRING LPARENNOSPACE expr COMMA expr COMMA expr RPAREN { ESubString($3, $5, $7, full_span()) }

printf:
  | PRINTF LPARENNOSPACE expr COMMA exprs RPAREN { EPrintf($3, $5, full_span()) }
