...
%token <string> ID
%token <string> STRING
%token DEF ANDDEF ADD1 SUB1 LPARENSPACE LPARENNOSPACE RPAREN LBRACK RBRACK LET IN EQUAL COMMA PLUS CONCAT MINUS TIMES IF COLON ELSECOLON EOF PRINTSTACK TRUE FALSE ISSTRING LEN ISBOOL ISNUM ISTUPLE EQEQ LESSSPACE GREATER LESSEQ GREATEREQ AND OR NOT COLONEQ SEMI NIL LAMBDA BEGIN END SHADOW REC UNDERSCORE
